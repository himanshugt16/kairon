...
class CatalogIntegrationEvent(EventsBase):
    """
    ...
    def validate(self, **kwargs):
        ...
        CatalogIntegrationLogProcessor.is_catalog_sync_allowed(self.bot)
        CatalogIntegrationLogProcessor.is_event_in_progress(self.bot)
        # CatalogIntegrationLogProcessor.is_limit_exceeded(self.bot)
    ...
    def enqueue(self, **kwargs):
        ...
        CatalogIntegrationLogProcessor.add_log(self.bot, self.user, self.integration, self.event_type, event_status=EVENT_STATUS.ENQUEUED.value)
        try:
            Utility.request_event_server(EventClass.catalog_integration, payload)
        except Exception as e:
            CatalogIntegrationLogProcessor.delete_enqueued_event_log(self.bot)
            raise e
    ...
    async def execute(self, **kwargs):
        ...
        try:
            ...
            knowledge_vault_data = CatalogIntegrationLogProcessor.extract_knowledge_vault_data(self.data, self.event_type)
            CatalogIntegrationLogProcessor.add_log(self.bot, self.user, event_status=EVENT_STATUS.VALIDATING.value)
            restaurant_name = self.data["restaurants"][0]["details"]["restaurantname"]
            error_summary = cognition_processor.validate_data("id", f"catalog_{restaurant_name}",
            ...
            if error_summary:
                ...
                status = "Failure"
            CatalogIntegrationLogProcessor.add_log(self.bot, self.user, validation_errors=error_summary,
                                                event_status=EVENT_STATUS.SAVE.value)
            if initiate_import:
                result = await cognition_processor.upsert_data_new("id", f"catalog_{restaurant_name}",
            ...
            CatalogIntegrationLogProcessor.add_log(self.bot, self.user, event_status=EVENT_STATUS.COMPLETED.value, status=status)
        except Exception as e:
            logger.error(str(e))
